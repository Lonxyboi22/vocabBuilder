...
function definition(word){
    ...
    fetch(dictionaryapi).then(function(response){
        if(response.ok){
            ...
            alert("Error - word: "+word+" "+response.statusText);
            wordIsMissing = true;
...
// function to make sure suggestedWords are in our dictionary
var wordIsMissing = false;
var missingWordList = [];
var wordInDictionary = function (){
    let i = 0;
    let start = 0;
    for (j=start+0; j<suggestedWords.length; j++){
        definition(suggestedWords[j]);
